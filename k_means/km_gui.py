...
class TkGUI:
    ...
    def makegui(self):
        self.processButton = tk.Button(text=' Process k-Means ', command=self.getKMeans, bg='brown', fg='white', font=('helvetica', 10, 'bold'))
    ...
    def getKMeans(self):
        ...
        for k in range(self.k):
            ax1.plot(self.centroids[k][2],self.centroids[k][3],marker='*',color=(0,0,1))
        color_list = np.array(self.km.color_list1)
        ax1.scatter(self.df['petal_length'], self.df['petal_width'], c=self.cluster, s=50, alpha=0.5)
        ax2.scatter(self.df['petal_length'], self.df['petal_width'], c=self.km.color_list1, s=50, alpha=0.5)
        self.scatter1 = FigureCanvasTkAgg(self.figure1, self.root) 
        self.scatter1.draw()
